...
class UnconditionRot(nn.Module):
    def __init__(self):
        ...
        self.rot = nn.Parameter(
            torch.randn((1, 4, 4)) * 1e-3 + torch.eye(4).unsqueeze(0)
        )
    ...
    def forward(self, rotation, permute=None, feature=None):
        ...
        quat = RotationConverterTorch.convert_from_to(
            rotation.reshape(-1, 3, 3), "matrix", "quaternion"
        )
        rotated_quat = (rot_mat @ (quat.reshape(-1, 4, 1))).reshape(-1, 4)
        result = RotationConverterTorch.convert_from_to(
            rotated_quat, "quaternion", "matrix"
        ).reshape(rot_shape)
...
class ConditionRot(nn.Module):
    ...
    def forward(self, rotation, permute=None, feature=None):
        mat_4d = self.net(feature).reshape(-1, 4, 4) + torch.eye(
        ...
        rot_shape = rotation.shape
        quat = RotationConverterTorch.convert_from_to(
            "matrix", "quaternion", rotation.reshape(-1, 3, 3)
        )
        rotated_quat = (rot_mat @ (quat.reshape(-1, 4, 1))).reshape(-1, 4)
        result = RotationConverterTorch.convert_from_to(
            "quaternion", "matrix", rotated_quat
        ).reshape(rot_shape)
        return result, torch.zeros(
            (rotation.shape[0],), device=rotation.device, dtype=rotation.dtype
        )
...
def calculate_9_l(mat, rot):
    ...
    return torch.einsum("nab,nbc->nac", U, V.transpose(-1, -2)), torch.zeros(
        (rot.shape[0],), device=rot.device, dtype=rot.dtype
    )
...
def calculate_9_r_smith(mat, rot, inverse=False):
    ...
    mat_1 = mat[..., 1] - (mat_0 * mat[..., 1]).sum(dim=-1, keepdim=True) * mat_0
    ...
    return torch.einsum("nab,nbc->nac", rot, new_mat), torch.zeros(
        (rot.shape[0],), device=rot.device, dtype=rot.dtype
    )
